...
### Processo (TDD con uso di GITFLOW)
...
* La ricezione via input di un programma contenente solo numeri (**interi positivi** separati da uno spazio), li impila sullo stack.
```java
...
* L'interprete è in grado di eseguire le somme: l'operatore `+`  sostituisce i due operandi in cima allo stack con la loro somma. Attenzione al '+5' che è da considerarsi sbagliato.
```java
